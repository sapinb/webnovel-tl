...
function chineseToArabic(str: string): number | null {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    if (chineseNumMap[char] !== undefined) {
      ...
      if (currentDigit === 0) {
        // Handle cases like "百" -> 100 (implicit "一")
        currentDigit = 1;
      }
